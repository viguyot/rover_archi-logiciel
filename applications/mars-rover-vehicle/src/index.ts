...
function generateRandomObstacles(
  ...
  while (obstacles.length < count && attempts < maxAttempts) {
    ...
    const alreadyExists = obstacles.some((existing) =>
      positionsEqual(existing, newObstacle)
    );
    if (alreadyExists) {
      attempts++;
      continue;
...
function parseArguments(): VehicleConfig {
  ...
  for (let i = 0; i < args.length; i += 2) {
    ...
    switch (key) {
      ...
      case "--planet-height":
        ...
        break;
      case "--log-wrapping":
        config.loggingConfig!.enableWrappingLogs =
          value.toLowerCase() === "true";
        break;
      case "--log-movement":
        config.loggingConfig!.enableMovementLogs =
          value.toLowerCase() === "true";
        break;
      case "--log-commands":
        config.loggingConfig!.enableCommandLogs =
          value.toLowerCase() === "true";
        break;
      case "--log-obstacles":
        config.loggingConfig!.enableObstacleLogs =
          value.toLowerCase() === "true";
        break;
      case "--help":
        showHelp();
  ...
  if (planetDimensionsChanged) {
    const randomConfig = generateRandomPlanetConfig(
...
function validateConfig(config: VehicleConfig): boolean {
  ...
  if (
    ...
    config.initialPosition.x >= config.planetConfig.width ||
    config.initialPosition.y < 0 ||
    config.initialPosition.y >= config.planetConfig.height
  ) {
  ...
  const obstacle = config.planetConfig.obstacles.find(
    (obs) =>
      obs.x === config.initialPosition.x && obs.y === config.initialPosition.y
  );
  if (obstacle) {
    console.warn(
      "⚠️ Position initiale en conflit avec un obstacle, régénération des obstacles..."
    );
...
function main(): void {
  ...
  try {
    ...
    console.log(
      `   📍 Position: (${config.initialPosition.x}, ${config.initialPosition.y})`
    );
    console.log(`   🧭 Direction: ${config.initialDirection}`);
    console.log(
      `   🌍 Planète: ${config.planetConfig.width}x${config.planetConfig.height}`
    );
    ...
    console.log(
      "✅ Mars Rover Vehicle prêt - En attente de connexions Mission Control"
    );
